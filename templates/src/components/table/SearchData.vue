<template>
	<q-drawer v-model="searchOpen" side="right" overlay elevated>
		<div class="q-pa-md">
			<q-input
				outlined
...
watch(
	() => store.getsearchOpen,
	(newValue) => {
		searchOpen.value = newValue;
	}
);
watch(
	() => searchOpen.value,
	(newValue) => {
		store.searchOpenChange(newValue);
		if (searchOpen.value == false) {
...
const slt = ref([]);
inputData.value = itemData.value?.filter(
	(item) => item.search && !item.toggle && !item.select
);
toggleData.value = itemData.value?.filter(
	(item) => item.search && item.toggle != undefined
);
slt.value = itemData.value?.filter((item) => item.search && item.select);
...
const filterFn = (val, update, abort) => {
	...
	//此处调接口取数据
	update(() => {
		api.get(props.dataUrl + "?" + selectUrl.value + "=" + val);
...
//确定
const clickHandel = () => {
	// console.log(props.columnData, "全部数据");
